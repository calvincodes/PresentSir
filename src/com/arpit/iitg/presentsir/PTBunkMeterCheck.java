...
public class PTBunkMeterCheck extends Activity {
	...
	private void check_false_alarm() {
		...
		SharedPreferences open_tt_to_bm_check = getApplicationContext().getSharedPreferences("PTTimeTable",Context.MODE_PRIVATE);
		
		switch(hours){
			case 7: slot_to_show = "_morn5_45to6_30"; break;
			case 19: slot_to_show = "_eve5_45to6_30"; break;
		}
		
		if(open_tt_to_bm_check.contains(today+slot_to_show) && (open_tt_to_bm_check.getInt("bm_check",0) == 1)){
	        ...
	        if(sound_val == 1){ // Vib + Mel
	 		...
	 		// ID - 301
	 		auto_bm_count_update = new Intent(getBaseContext(), AutoUpdatePTBunkMeter.class);
	 		auto_bm_count_update.putExtra("hours", hours);
	 		pi_auto_bm_count_update = PendingIntent.getActivity(getBaseContext(), 301, auto_bm_count_update, PendingIntent.FLAG_UPDATE_CURRENT);
	...
	private void count_for_bm(String day) {
		...
		switch(hours){
			case 7: alert_msg = "Did you BUNK the morning PT class?"; break;
			case 19: alert_msg = "Did you BUNK the evening PT class?"; break;
		}
	...
	protected void cancel_auto_update_alarm() {
		...
		try {
			Intent intent = new Intent(getBaseContext(), AutoUpdatePTBunkMeter.class);
			PendingIntent displayIntent = PendingIntent.getActivity(getApplicationContext(), 301, intent, PendingIntent.FLAG_UPDATE_CURRENT);
	...
	protected void increase_bunk_count() {
		...
		SharedPreferences open_tt_to_incr_bm = getApplicationContext().getSharedPreferences("PTTimeTable",Context.MODE_PRIVATE);
	...
	private void go_to_home() {
		Intent app_home_screen = new Intent(PTBunkMeterCheck.this, AppHome.class);
